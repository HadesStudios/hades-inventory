...
function RetrieveComponents()
	...
	Default = exports["hades-base"]:FetchComponent("Default")
	Inventory = exports["hades-base"]:FetchComponent("Inventory")
	EntityTypes = exports["hades-base"]:FetchComponent("EntityTypes")
	...
	Crafting = exports["hades-base"]:FetchComponent("Crafting")
	...
	Drugs = exports["hades-base"]:FetchComponent("Drugs")
	Robbery = exports["hades-base"]:FetchComponent("Robbery")
	Laptop = exports["hades-base"]:FetchComponent("Laptop")
end

AddEventHandler("Core:Shared:Ready", function()
	exports["hades-base"]:RequestDependencies("Inventory", {
		"Database",
		"Callbacks",
		"Sequence",
		"Fetch",
		...
		if type(f) == 'table' and f.Account ~= nil then
			...
			_govAccount = nil
			Logger:Warn("Inventory", "Failed to fetch government bank account")
		end
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
	exports["hades-base"]:RegisterComponent("Inventory", INVENTORY)
end)
...
INVENTORY = {
	...
	Items = {
		...
		HasAnyItems = function(self, source, items)
			local player = exports["hades-base"]:FetchComponent("Fetch"):Source(source)
			local char = player:GetData("Character")
			local charId = char:GetData("SID")
...
-- Give Item System (server-side handlers)
local GivingItemInProgress = {}

RegisterNetEvent("Inventory:Server:TryGiveItem", function(data)
	Player(source).state.isGivingItem = data
end)

RegisterNetEvent("Inventory:Server:TryGiveItem2", function(targetId)
    local _src = source
    if GivingItemInProgress[_src] then
        print(("Player %s tried to give an item while still in progress."):format(_src))
        return
    end

    GivingItemInProgress[_src] = true

    local success, err = pcall(function()
        local ItemGivin = Player(_src).state.isGivingItem
        Player(_src).state.isGivingItem = nil
        if not ItemGivin then
            error("No item to give.")
        end
        local targetPlayer = Fetch:Source(tonumber(targetId))
        ...
        if not targetCharacter then
            error("Target has no character.")
        end
        ItemGivin.ownerTo = targetCharacter:GetData("SID")
        ItemGivin.invTypeTo = 1
        local itemExist = itemsDatabase[ItemGivin.name]
        if not itemExist then
            error("Item not found in database: " .. tostring(ItemGivin.name))
        end
        local invWeight = Inventory.Items:GetWeights(ItemGivin.ownerTo, ItemGivin.invTypeTo)
        local totWeight = invWeight + (ItemGivin.countTo * itemExist.weight)
        if totWeight < getCapacity(ItemGivin.invTypeTo) and ItemGivin.ownerFrom ~= ItemGivin.ownerTo then
            local freeSlots = Inventory:GetFreeSlotNumbers(ItemGivin.ownerTo, ItemGivin.invTypeTo)
            if #freeSlots >= 1 then
                ItemGivin.slotTo = freeSlots[1]
                print("[Server] Giving item. SlotTo:", ItemGivin.slotTo)
                TriggerClientEvent("Inventory:Client:TryGiveItem2", _src, ItemGivin)
            end
        end
    end)
...
RegisterNetEvent("Inventory:Server:UpdateSettings", function(data)
	...
	local char = Fetch:Source(src):GetData("Character")
	if char then
		char:SetData("InventorySettings", data)
	end
end)