...
function LoadShops()
	CreateThread(function()
		...
		for k, v in ipairs(_shops) do
			...
			local ent = LoadedEntitys[v.entityId]
			if ent then
				v.restriction = ent.restriction
			else
				v.restriction = nil
				Logger:Warn("Inventory", string.format("Missing entity type for shop %s (entityId: %s)", tostring(id), tostring(v.entityId)))
			end
			shopLocations[string.format("shop:%s", id)] = v
		end

		for k, v in pairs(_entityTypes) do
			-- guard: ensure f is valid before indexing
			if type(f) == 'table' and f.Account ~= nil then
				storeBankAccounts[v.id] = f.Account
			else
				storeBankAccounts[v.id] = nil
				Logger:Warn("Inventory", "Failed to load default store bank account for entity type " .. tostring(v.id))
			end
		end
...
function RegisterCommands()
	...
	Chat:RegisterAdminCommand("clearinventory", function(source, args, rawCommand)
		...
		local player = exports["hades-base"]:FetchComponent("Fetch"):SID(sid)
		if player then
